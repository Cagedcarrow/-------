function shovel_tcp_ik_improve()
...
% - On RUN click: real-time command line logs for IK + animation progress
    ...
    fprintf('[INFO] version=2026-04-17-parabola-v8-anti-jump\n');
    ...
    params.qJumpWeight = 0.10;         % 关节连续性惩罚权重
    params.qJumpWarnDeg = 25.0;        % 单步关节变化告警阈值(每轴最大值)
    params.qJumpRejectDeg = 50.0;      % 单步关节变化强惩罚阈值(每轴最大值)
    params.primaryPosTol = 0.03;       % m: 主种子可接受位置误差
    params.primaryOriTolDeg = 10.0;    % deg: 主种子可接受姿态误差
    params.primaryYAlignTolDeg = 10.0; % deg: 主种子可接受y轴对齐误差
    ...
    function onResetParams(~, ~)
        ...
        params.pitchBiasDeg = -12.0;
        params.assemblyOffsetDeg = 0.0;
        params.qJumpWeight = 0.10;
        params.qJumpWarnDeg = 25.0;
        params.qJumpRejectDeg = 50.0;
        params.primaryPosTol = 0.03;
        params.primaryOriTolDeg = 10.0;
        params.primaryYAlignTolDeg = 10.0;
        params.framePause = 0.03;
        params.reachRadius = estimateReachRadius(robot, baseLink, endEffector, 0.20);
        params.flipToolZ = true;
    ...
    function onRun(~, ~)
        ...
        fprintf('[RUN] anti-jump: qW=%.3f, warn=%.1fdeg, reject=%.1fdeg, primaryTol=[%.3fm %.1fdeg %.1fdeg]\n', ...
            params.qJumpWeight, params.qJumpWarnDeg, params.qJumpRejectDeg, ...
            params.primaryPosTol, params.primaryOriTolDeg, params.primaryYAlignTolDeg);
        ...
        for i = 1:nPts
            ...
            % 防姿态基向量突变: 与上一帧方向相反时整体翻转x/z
            if ~isempty(prevXAxis_b)
                if dot(xAxis_b, prevXAxis_b) < 0 || dot(zAxis_b, prevZAxis_b) < 0
                    xAxis_b = -xAxis_b;
                    zAxis_b = -zAxis_b;
                end
            end
            ...
            qRef = qPrev;
            seedTag = 'P';
            [qBest, infoBest, ePos, eOriDeg, eYAlignDeg, eQJumpDeg, scoreBest] = ...
                solveOneTarget(T_tar, qPrev, yTarWorld, qRef);

            % 主种子不可靠时再回退，避免频繁跨分支
            if ~isSolveAcceptable(infoBest, ePos, eOriDeg, eYAlignDeg) || eQJumpDeg > params.qJumpRejectDeg
                [qFb, infoFb, ePosFb, eOriFb, eYAlignFb, eQJumpDegFb, scoreFb] = ...
                    solveOneTarget(T_tar, [qCurrent; qHomeLocal], yTarWorld, qRef);
            ...
            qJumpDeg(i) = eQJumpDeg;
            statusArr(i) = string(sprintf('%s|%s', seedTag, infoBest.Status));

            if eQJumpDeg > params.qJumpWarnDeg
                jumpWarnCount = jumpWarnCount + 1;
                fprintf('[JUMP-WARN %03d] stepMax=%.2fdeg (>%.2fdeg), seed=%s, x=%.3f\n', ...
                    i, eQJumpDeg, params.qJumpWarnDeg, seedTag, x_b);
            end

            if i <= 5 || mod(i,10) == 0 || i == nPts
                fprintf(['[IK %03d/%03d] x=%.3f k=%.3f tan=%.2fdeg | posErr=%.4fm oriErr=%.2fdeg ' ...
                    'yAlignErr=%.2fdeg qJumpMax=%.2fdeg seed=%s status=%s\n'], ...
                    i, nPts, x_b, k, tanAngleDeg, ePos, eOriDeg, eYAlignDeg, eQJumpDeg, seedTag, infoBest.Status);
            end
        ...
        fprintf('[RUN] continuity: qJumpMax mean=%.2f max=%.2f (deg) | fallback=%d/%d | jumpWarn=%d\n', ...
            mean(qJumpDeg), max(qJumpDeg), fallbackCount, nPts, jumpWarnCount);
    ...
    function [qBest, infoBest, posBest, oriBestDeg, yAlignBestDeg, qJumpBestDeg, scoreBest] = solveOneTarget(Ttar, seeds, yTarWorld, qRef)
        ...
        for s = 1:size(seeds,1)
            ...
            qTry = nearestEquivalentToRef(qTry, qRef, jointLimits);
            ...
            dqDeg = abs(rad2deg(qTry - qRef));
            qJumpDeg = max(dqDeg);
            jumpPenalty = max(0, qJumpDeg - params.qJumpRejectDeg);

            score = posErr + 0.01 * oriErrDeg + 0.02 * yAlignErrDeg + ...
                params.qJumpWeight * deg2rad(qJumpDeg) + 10.0 * deg2rad(jumpPenalty);
            if score < scoreBest
                scoreBest = score;
                qBest = qTry;
                infoBest = infoTry;
...
function qOut = nearestEquivalentToRef(qIn, qRef, lim)
    qOut = qIn;
    n = min([numel(qIn), numel(qRef), size(lim,1)]);
    for i = 1:n
